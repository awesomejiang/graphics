...
class card_spider(CrawlSpider):
	...
	#codes for benchmark		
	rules = (
		Rule( LinkExtractor(allow = '/cards/27266-overloaded-mechazod'), callback = 'parse_item' ),
		Rule( LinkExtractor(restrict_xpaths = u"//a[@rel='next']"), callback = 'parse_next' )
			)
	...
	def parse_item(self, response):
		...
		item['Name'] = temp if temp else None
		
		for temp in sel.xpath('aside/ul/li'):
			text = temp.xpath('text()').extract()
			if 'Type: ' in text:
				item['Type'] = temp.xpath('a/text()').extract()
			elif 'Class: ' in text:
				item['Class'] = temp.xpath('a/text()').extract()
			elif 'Rarity: ' in text:
				item['Rarity'] = temp.xpath('a/text()').extract()
			elif 'Set: ' in text:
				item['Set'] = temp.xpath('a/text()').extract()

		temp = sel.xpath('aside/ul/li/span/text()').extract()
		item['Tip'] = temp[:1] if temp else None
		
		temp = sel.xpath('div[@class="card-info u-typography-format"]//span/text()').extract()
		item['Skill'] = temp if temp else None
		...
		if  '' in temp:
			...
			item['image_urls'] =  temp if temp else None
